...
struct Parameters
{
	...
	float epsilon;
	float filter_param1;
	int windR; // i.e., W_p in the author's paper.
	int neighborNum;
	...
	Parameters(
			float lambda = 20, 
			int windR = 20, 
			std::string filterName = "BF", 
			float filter_param1 = 10
			)
		: alpha(0.9f)
		, omega(10.0f)
		, th_grad(2.0f)
...
public:
	...
	virtual void ComputeUnaryPotentialWithoutCheck(
			const cv::Rect& filterRect, 
	...
	virtual void ComputeUnaryPotential(const cv::Rect& filterRect, const cv::Rect& targetRect, const cv::Mat& costs, const Plane& plane, 
			//Reusable& reusabl = Reusable(), 
			Reusable& reusabl, 
			int mode = 0) const {};
};
...
public:
	...
	void ComputeUnaryPotentialWithoutCheck(const cv::Rect& filterRect, const cv::Rect& targetRect, const cv::Mat& costs, const Plane& plane, 
			//Reusable& reusable = Reusable(), 
			Reusable& reusable, 
	...
	void ComputeUnaryPotential(const cv::Rect& filterRect, const cv::Rect& targetRect, const cv::Mat& costs, const Plane& plane, 
			//Reusable& reusable = Reusable(), 
			Reusable& reusable, 
			int mode = 0) const override
	{
