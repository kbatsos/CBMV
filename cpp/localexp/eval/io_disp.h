...
public:
  ...
  inline bool isValid (const int32_t u,const int32_t v) {
    return (data_[v*width_+u]>=0 and !std::isinf(data_[v*width_+u]));
  }
  ...
  png::image<png::rgb_pixel> errorImage (
			DisparityImage &D_noc,
			DisparityImage &D_occ,
      ...
      for (int32_t u=1; u<width()-1; u++) {
        if (D_occ.isValid(u,v)) {
          ...
          if (log_colors) {
            ...
            float d_err = fabs(getDisp(u,v)-D_occ.getDisp(u,v))/5.0;
				    d_err = d_err > 1.0 ? d_err: 1.0 ; 
  ...
  // compute error map of current image, given the non-occluded and occluded
  // ground truth disparity maps. stores result as color png image.
  void errorImage_pkls(
			DisparityImage &D_noc,
			DisparityImage &D_occ,
			const std::string & imagefn){

		//printf("red means error\n");
    ...
    for (int32_t v=0; v<height(); v++) {
      for (int32_t u=0; u<width(); u++) {
        if (D_occ.isValid(u,v)) {
					...
					float d_err = fabs(getDisp(u,v)-D_occ.getDisp(u,v));
					float d_mag = fabs(D_occ.getDisp(u,v));
          bool is_err = (d_err > 3.0) && (20.0*d_err > d_mag);
					
					if(is_err){
					  image.at<cv::Vec3b>(v,u)[0] = 0;//blue
					  image.at<cv::Vec3b>(v,u)[1] = 0;//green
					  image.at<cv::Vec3b>(v,u)[2] = 255;//red
					}

					else{
					  image.at<cv::Vec3b>(v,u)[0] = 0;//blue
					  image.at<cv::Vec3b>(v,u)[1] = 255;//green
					  image.at<cv::Vec3b>(v,u)[2] = 0;//red
					}
					
					if (!D_noc.isValid(u,v)){
					  image.at<cv::Vec3b>(v,u)[0] = 255;//blue
					  image.at<cv::Vec3b>(v,u)[1] = 0;//green
					  image.at<cv::Vec3b>(v,u)[2] = 0;//red
					}
				} 
