...
namespace cvutils
{
	...
	{
		cv::Mat m1m2 = m1.mul(m2);//element-wise multiplication;
		m1m2 = m1m2.reshape(1, m1.rows*m1.cols);
		cv::Mat m1m2dot;
		// Note:
		// void cv::reduce(InputArray src, OutputArray dst, int dim, int rtype, int dtype=-1 )
		// args:
		// * dim : dimdimension index along which the matrix is reduced. 
		//         0 means that the matrix is reduced to a single row. 
		//         1 means that the matrix is reduced to a single column.
		cv::reduce(m1m2, m1m2dot, 1, cv::REDUCE_SUM);//Reduces a matrix to a single colume vector, due to dim = 1 here.
		return m1m2dot.reshape(1, // New number of channels. If the parameter is 0, the number of channels remains the same.
				m1.rows // New number of rows.If the parameter is 0, the number of rows remains the same.
				);
	}
