...
public:
  ...
  // override: Specifies that a virtual function overrides another virtual function.
	// The identifier override, if used, appears immediately after the declarator 
	// in the syntax of a member function declaration or a member function definition 
...
public:
    ...
    //filter->R = GuidedImageFilter<Type>::R;
		// changed by CCJ;
		// see > o fix this, you need to specify Base<T>::data_.clear() or this->data_.clear(). As for why this happens, see here.
		// at https://stackoverflow.com/questions/12032345/protected-member-is-not-declared-in-this-scope-in-derived-class
		// To fix this, you need to specify Base<T>::data_.clear() or this->data_.clear().
		filter->R = this -> R;
		filter->eps  = this -> eps;
